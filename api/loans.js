...
export default async function handler(req, res) {
  ...
  // parts: ["api", "loans"] or ["api", "loans", "staffId"] or ["api", "loans", "staffId", "payment"]
  const staffId = parts[2];
  const isPayment = parts[3] === 'payment';
  ...
  if (req.method === 'PUT') {
    if (!staffId) return err(res, 'Staff ID required', 400);
  ...
  if (req.method === 'POST') {
    if (!staffId || !isPayment) return err(res, 'Invalid request', 400);
